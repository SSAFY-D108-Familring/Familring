...
class LoginViewModel @Inject constructor(
    ...
    private suspend fun loginWithKakaoTalk() {
        try {
            ...
            val token = suspendCancellableCoroutine<OAuthToken?> { continuation ->
                    ...
                    viewModelScope.launch {
                        try {
                            ...
                            userRepository.login(
                                ...
                                when (response) {
                                    is ApiResponse.Success<JwtToken> -> {
                                    ...
                                    is ApiResponse.Error -> {
                                        ...
                                        _loginState.value = LoginState.Error("로그인 실패: ${response.errorMessage}")
                                        _loginEvent.emit(
                                            LoginEvent.Error(
                            ...
                            _loginEvent.emit(LoginEvent.Error(errorMessage = e.message ?: "서버 통신 실패"))
                        }
                    }
                }
            }
